...
namespace WOW_Fusion.Services
{
    internal class CommonService
    {
        ...
        public static async Task<List<string>> WOProcessByOrg(string organizationId)
        {
            try
            {
                Task<string> tskWorkOrders = APIService.GetRequestAsync(String.Format(EndPoints.WOListByOrg, organizationId));
                string response = await tskWorkOrders;
                if (string.IsNullOrEmpty(response))
                {
                    return null;
            ...
            {
                MessageBox.Show(ex.Message, "[Error] Lista de ordenes por organización", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return null;
            }
        }
